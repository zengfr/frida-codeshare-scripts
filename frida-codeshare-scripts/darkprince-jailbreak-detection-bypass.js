...
//1828396841 @sridharas04/darkprince-jailbreak-detection-bypass
...
if (ObjC.available) {
    ...
    // Dynamic Lib Bypass
    if (ObjC.available) {
        var dyldGetImageName = Module.findExportByName(null, '_dyld_get_image_name');
        Interceptor.attach(dyldGetImageName, {
            onLeave: function(retval) {
                // Check if retval is valid
                if (!retval.isNull()) {
                    try {
                        // Read the original image name
                        var originalValue = Memory.readUtf8String(retval);

                        // Check if the current image name is in the list of paths to hide
                        var index = jbPaths.indexOf(originalValue);
                        if (index !== -1) {
                            // Replace the return value with a placeholder or an empty string
                            Memory.writeUtf8String(retval, "/usr/lib/libsqlite3.dylib");

                            // Add colors for better visibility
                            console.log("\x1b[31mOriginal Path: " + originalValue + "\x1b[0m");
                            console.log("\x1b[32mBypassed and Path Modified\x1b[0m");
                        }
                    } catch (e) {
                        // Handle exceptions, e.g., invalid memory access
                        //console.log("\x1b[31mError: " + e.message + "\x1b[0m");
                    }
                } else {
                    console.log("\x1b[31mError: retval is null\x1b[0m");
                }
            }
        });
    } else {
