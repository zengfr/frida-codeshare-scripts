...
//hash:-556065935 @chanmyaekozin-ucsy/thief
/*
...
function enableOtpEditField() {
    const EditText = Java.use("android.widget.EditText");

    EditText.setEnabled.implementation = function(value) {
        console.log("✏️ setEnabled() called → forcing TRUE");
        return this.setEnabled(true);
    };

    EditText.setFocusable.implementation = function(value) {
        console.log("✏️ setFocusable() called → forcing TRUE");
        return this.setFocusable(true);
    };

    EditText.setFocusableInTouchMode.implementation = function(value) {
        console.log("✏️ setFocusableInTouchMode() called → forcing TRUE");
        return this.setFocusableInTouchMode(true);
    };

    EditText.setInputType.implementation = function(type) {
        console.log("✏️ setInputType() overridden");
        return this.setInputType(type); // allow normal behavior
    };

    EditText.setText.overload('java.lang.CharSequence').implementation = function(txt) {
        const str = txt.toString();
        if (str.length > 3 && str.match(/^\d+$/)) {
            console.log("🛡️ Preventing auto OTP input:", str);
            return; // block SMS auto-fill
        }
...
Java.perform(function () {
    // Hook OkHttp Request body to log POST parameters
    ...
    RequestBody.writeTo.implementation = function (sink) {
        ...
        if (bodyStr.includes("refresh_token")) {
            console.log("📦 Intercepted HTTP Body:");
            console.log(bodyStr);
        }
        return this.writeTo(sink);
    };

    // Hook OkHttp Request object to get URL and headers
    ...
    RequestBuilder.build.implementation = function () {
        ...
        if (url.includes("/token")) {
            console.log("🌐 Intercepted Refresh Token Request URL:");
            console.log(url);
            console.log("📋 Headers:");
    ...
    // Optional: Simulate token expiry by faking system time
    var System = Java.use("java.lang.System");
    System.currentTimeMillis.implementation = function () {
        var now = this.currentTimeMillis();
        var fake = now + 2678400000; // Add 31 days
        return fake;
    };

    console.log("🔍 Refresh token interceptor + fake time patch loaded");
});
...
Java.perform(function () {
    console.log("🚫 Blocking /logout requests...");

    const OkHttpClient = Java.use("okhttp3.OkHttpClient");
    const RequestBuilder = Java.use("okhttp3.Request$Builder");

    // Intercept request building to change URL if it contains /logout
    RequestBuilder.url.overload('java.lang.String').implementation = function (url) {
        if (url.includes("/logout")) {
            console.warn("🛑 Blocked logout URL:", url);

            // Option 1: Replace with harmless endpoint
            url = url.replace("/logout", "/healthcheck");

            // Option 2: Comment above and uncomment this to cancel request entirely
            // throw new Error("Blocked /logout request intentionally");
        }
        return this.url(url);
    };

    // Also patch newRequest in case they use Request directly
    const Request = Java.use("okhttp3.Request");
    Request.newBuilder.implementation = function () {
        const requestUrl = this.url().toString();
        if (requestUrl.includes("/logout")) {
            console.warn("🛑 Intercepted request to /logout");
        }
        return this.newBuilder();
    };

    console.log("✅ Logout interception is active.");
});


enableOtpEditField();
console.log("✅ All bypasses + OTP edit patch enabled");

Java.perform(function () {
  var System = Java.use("java.lang.System");
  System.currentTimeMillis.implementation = function () {
    return this.currentTimeMillis() + 2678400000; // 31 days ahead
  };
});
console.log("✅  Skipped Timer");

Java.perform(function () {
    console.log(">> Unity hook started");

    // Hook Unity button click events
    var UnityEngine_UI_Button = Java.use("com.unity3d.player.UnityPlayerActivity"); // may vary
    UnityEngine_UI_Button.onClick.implementation = function () {
        console.log("[*] Button clicked - possibly started a level");
        return this.onClick();
    };

    // Hook UnityWebRequest.SendWebRequest
    try {
        var UnityWebRequest = Java.use("com.unity3d.player.UnityWebRequest"); // fallback if Proguarded
        UnityWebRequest.SendWebRequest.implementation = function () {
            console.log(">> UnityWebRequest Sent");
            var url = this.get_url();
            console.log("   URL: " + url);
            return this.SendWebRequest();
        };
    } catch (e) {
        console.warn("UnityWebRequest not found or obfuscated");
    }

    // Hook WWW constructor if used
    try {
        var WWW = Java.use("com.unity3d.player.WWW"); // older Unity builds
        WWW.$init.overload('java.lang.String').implementation = function (url) {
            console.log(">> WWW called with URL: " + url);
            return this.$init(url);
        };
    } catch (e) {
        console.warn("WWW not found or not used");
    }

    // Hook any interesting game-related method if names are known
    // E.g. 'submitScore', 'claimReward', 'startGame', etc.
});
//https://github.com/zengfr/frida-codeshare-scripts QQGroup: 143824179 .
//hash:-556065935 @chanmyaekozin-ucsy/thief
